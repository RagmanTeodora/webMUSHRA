...
MushraAudioControl.prototype.initAudio = function() {
  ...
  this.dummyBufferSource.loop = false; // true;
...
MushraAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  // Get the output buffer (where we'll write processed audio)
  var outputBuffer = audioProcessingEvent.outputBuffer;
  // Get the input buffer (in this case, the source audio samples)
  var inputBuffer = audioProcessingEvent.inputBuffer;
  
  // Reference to the currently loaded stimulus (audio clip)
  var stimulus = this.audioStimulus;
  var sample;
  var ramp; // Used for fade-in/out volume scaling
  var outputData;
  var channel;
  
  // --- CASE 1: No audio to play or playback is stopped ---
  if (stimulus === null || this.audioPlaying === false) {
    // Fill the output buffer with zeros (silence)
  ...
  // --- MAIN AUDIO PROCESSING LOOP PER CHANNEL ---
  for (channel = 0; channel < this.reference.getAudioBuffer().numberOfChannels; ++channel) {
    outputData = outputBuffer.getChannelData(channel); // Destination samples
    inputData = audioBuffer.getChannelData(channel);   // Source samples
    ...
    var a = []; // Seems unused here
    var b = []; // Seems unused here

    // Loop over each sample in the output buffer
    for (sample = 0; sample < outputBuffer.length; ++sample) {

      // NEW: wrap or stop BEFORE reading to avoid out-of-bounds
      ...
      // // Check if loop end is near → start fading out
      // if (loopingActive && (currentPosition == (this.audioLoopEnd - this.audioFadingMaxPosition))) {
      //   fadingActive = 1; // Fade-out mode
      //   this.audioFadingIn = this.audioStimulus;
      //   fadingCurrentPosition = 0;        
      // }
      
      // Only trigger fade-out if looping is enabled and we're actually going to loop
      if (loopingActive === true && this.audioFadingMaxPosition > 0 &&
        currentPosition == (this.audioLoopEnd - this.audioFadingMaxPosition)) {
          fadingActive = 1;
          this.audioFadingIn = this.audioStimulus;
          fadingCurrentPosition = 0;        
      }
      ...
      if (fadingActive == 1) {
        ...
        if (fadingCurrentPosition >= this.audioFadingMaxPosition) {          
          fadingActive = 2; // Switch to fade-in
          fadingCurrentPosition = 0;

          if (this.audioFadingIn === null) {
            // If no new audio to fade into, stop playback
            this.audioPlaying = false;
            fadingCurrentPosition = 0;
            fadingActive = 0;
            ...
            break;
          } else { // Switch to the new stimulus and refresh loop bounds to match its length
            stimulus = this.audioStimulus = this.audioFadingIn;
            var newBuf = stimulus.getAudioBuffer();

            this.audioMaxPosition = newBuf.length;
            this.audioLoopStart   = 0;
            this.audioLoopEnd     = newBuf.length;

            // Start the fade-in from the beginning of the new file
            currentPosition = this.audioLoopStart;
            inputData = newBuf.getChannelData(channel);
            }
        }

      // --- FADE-IN ---
      } else if (fadingActive == 2) {
        // Cosine-based fade-in curve
        ramp = 0.5 * (1 - Math.cos(Math.PI * (fadingCurrentPosition++) / (this.audioFadingMaxPosition - 1)));
        outputData[sample] = inputData[currentPosition++] * ramp;
      ...
      if (currentPosition >= this.audioLoopEnd) {
        ...
        if (loopingActive === false) {
          this.surpressLoop(); // Possibly notifies system loop ended
          this.audioPlaying = false;
          break;
        }
      }
  ...
  for (channel = 0; channel < outputBuffer.numberOfChannels; ++channel) {
    outputData = outputBuffer.getChannelData(channel);
    for (sample = 0; sample < outputBuffer.length; ++sample) {  
      outputData[sample] *= this.audioContext.volume;         
    }   
  }  
  
  // Save updated playback state
  ...
  // Send event back to application with current playback position
  var event = {
      name: 'processUpdate',
...
MushraAudioControl.prototype.play = function(_stimulus, _isReference) {
  ...
  if ((this.audioStimulus !== _stimulus || _isReference !== this.audioIsReferencePlaying) 	
      && this.audioStimulus !== null && this.audioPlaying !== false) {
    // Switching to a new stimulus while playing → fade out
    this.fadeOut(_stimulus);
    ...
    // NEW: set loop & max positions to this file's actual length
    if (this.audioStimulus) {
      const buf = this.audioStimulus.getAudioBuffer();
      const len = buf.length;

      this.audioMaxPosition = len;
      this.audioLoopStart   = 0;
      this.audioLoopEnd     = len;

      // Ensure current position is inside the new bounds
      if (this.audioCurrentPosition < this.audioLoopStart || 
          this.audioCurrentPosition > this.audioLoopEnd) {
        this.audioCurrentPosition = this.audioLoopStart;
      }
    }
